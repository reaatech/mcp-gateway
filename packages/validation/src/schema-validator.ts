...
type AjvErrorObject = ErrorObject;
...
class SchemaValidator {
  ...
  constructor() {
    this.ajv = new (AjvDefault as unknown as new (opts: Record<string, unknown>) => AjvInstance)({
      allErrors: true,
      verbose: true,
      coerceTypes: false,
      useDefaults: false,
  ...
  validateToolArguments(
    ...
    return this.formatErrors(
      (validator as { errors?: AjvErrorObject[] }).errors || [],
      'arguments',
    );
  }
   ...
   * Cache a tool schema
   */
  cacheToolSchema(toolName: string, schema: Record<string, unknown>, ttl?: number): void {
    const cacheKey = `tool:${toolName}`;
   ...
   */
  private formatErrors(errors: AjvErrorObject[], defaultField = 'params'): ValidationResult {
