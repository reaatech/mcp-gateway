...
describe('MemoryAuditStorage', () => {
  ...
  describe('query', () => {
    beforeEach(() => {
      storage.store(
        createAuditEvent('auth.success', 'req-1', { tenantId: 'tenant-a', tool: 'tool1' }),
      );
      storage.store(
        createAuditEvent('auth.failure', 'req-2', { tenantId: 'tenant-a', tool: 'tool2' }),
      );
      storage.store(
        createAuditEvent('tool.executed', 'req-3', { tenantId: 'tenant-b', success: true }),
      );
      storage.store(
        createAuditEvent('tool.executed', 'req-4', { tenantId: 'tenant-b', success: false }),
      );
    });
