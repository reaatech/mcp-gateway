...
import '@reaatech/mcp-gateway-observability';
...
export function createApp(options: CreateAppOptions = {}): GatewayApp {
  ...
  const perTenantRateLimit = async (
    ...
    const result = await rateLimiter.checkLimit(
      tenantId,
      tenant?.rateLimits as unknown as Parameters<typeof rateLimiter.getRemaining>[1],
    );
    if (!result.allowed) {
  ...
  const getAccessibleTenants = (
    req: Request,
  ): typeof listTenants extends () => infer T ? T : never => {
    ...
    const tenantId = req.authContext?.tenantId;
    return tenantId
      ? ([getTenant(tenantId)].filter(Boolean) as ReturnType<typeof listTenants>)
      : [];
  };
  ...
  admin.get('/upstreams', (req, res) => {
    ...
    if (
      requestedTenantId &&
      !isAdminRequest(req) &&
      requestedTenantId !== req.authContext?.tenantId
    ) {
      res.status(403).json({ error: 'Forbidden' });
      return;
    }
    ...
    const statuses = await Promise.all(
      tenants.map(async (t) => {
        const remaining = await rateLimiter.getRemaining(
          t?.tenantId,
          t?.rateLimits as unknown as Parameters<typeof rateLimiter.getRemaining>[1],
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        res.json(
          fanoutResult.finalResponse ?? { jsonrpc: '2.0', id: body.id ?? null, result: null },
        );
      } catch (error) {
        next(error);
      }
...
async function main(): Promise<void> {
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    await new Promise<void>((resolve) => {
      server.close(async () => {
        try {
          const otelMod = await import('@reaatech/mcp-gateway-observability');
        ...
        try {
          const { shutdownOAuthIntrospection } = await import('@reaatech/mcp-gateway-auth');
