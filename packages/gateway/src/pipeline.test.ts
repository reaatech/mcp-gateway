...
function createMockRes() {
  ...
  return res as unknown as {
    ...
    end: (chunk?: unknown, encoding?: unknown) => unknown;
...
describe('request-id middleware', () => {
  it('generates request ID when not provided', () => {
    ...
    middleware(
      ...
      res as unknown as import('express').Response,
      () => {},
      context as unknown as import('./types.js').PipelineContext,
    );
...
describe('timeout', () => {
  describe('withTimeout', () => {
    it('completes within timeout', async () => {
      const result = await withTimeout(async () => 'success', 1000, 'Timeout');
