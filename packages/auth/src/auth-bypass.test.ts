...
function makeTenantConfig(
  apiKeys: Array<{ keyHash: string; name: string; scopes: string[]; expiresAt?: number }>,
): TenantConfig {
  return {
...
describe('Security: Auth Bypass Attempts', () => {
  ...
  it('rejects empty API key', () => {
    const result = validateApiKey(
      '',
      makeTenantConfig([{ keyHash: validHash, name: 'test', scopes: ['tools:*'] }]),
    );
  ...
  it('rejects API key that matches hash prefix only', () => {
    ...
    const result = validateApiKey(
      validKey,
      makeTenantConfig([{ keyHash: partialHash, name: 'test', scopes: ['tools:*'] }]),
    );
    expect(result.valid).toBe(false);
  });
  ...
  it('does not match key against a different tenant hash', () => {
    ...
    const result = validateApiKey(
      validKey,
      makeTenantConfig([{ keyHash: otherHash, name: 'other-key', scopes: ['tools:*'] }]),
    );
    expect(result.valid).toBe(false);
  });
