...
describe('oauth-introspection', () => {
  ...
  it('returns error when client credentials missing', async () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.error).toContain('OAuth client secret env variable');
  });
  ...
  it('returns error when introspection URL not set', async () => {
    ...
    const tenantWithoutUrl: TenantConfig = {
      ...TEST_TENANT_OAUTH,
      auth: {
        oauth: { introspectionUrl: '', clientId: 'test', clientSecretEnv: 'OAUTH_CLIENT_SECRET' },
      },
    };
  ...
  it('getIntrospectionCacheStats returns size', async () => {
    const { getIntrospectionCacheStats, clearIntrospectionCache } = await import(
...
describe('api-key-validator', () => {
  it('produces consistent hashes', async () => {
    const { hashApiKey } = await import('./api-key-validator.js');
...
describe('auth-context', () => {
  it('createAuthContext creates context with required fields', async () => {
    const { createAuthContext } = await import('./auth-context.js');
  ...
  it('generateTokenFingerprintSync produces consistent fingerprints', async () => {
    const { generateTokenFingerprintSync } = await import('./auth-context.js');
