...
import {
  ...
  setUpstreamCaller,
  resetUpstreamCaller,
} from '../../src/fanout/fanout-router.js';
import { aggregateResponses } from '../../src/fanout/response-aggregator.js';
import { selectUpstreams, selectByHealth, selectWeightedRandom } from '../../src/fanout/upstream-selector.js';
import { isCircuitOpen, recordFailure, recordSuccess, filterHealthyUpstreams } from '../../src/fanout/failover-handler.js';
import type { UpstreamTarget, UpstreamResponse } from '../../src/fanout/types.js';
...
describe('failover-handler', () => {
  ...
  describe('filterHealthyUpstreams', () => {
    it('filters out upstreams with open circuits', () => {
      ...
      for (let i = 0; i < 5; i++) {recordFailure('unhealthy');}
...
describe('fanout-router', () => {
  ...
  describe('executeFanout', () => {
    it('returns empty result for empty upstreams', async () => {
      const result = await executeFanout([], { jsonrpc: '2.0', method: 'test', id: 1 }, 'first-success');
    ...
    it('stops after first successful response in first-success mode', async () => {
      ...
      const result = await executeFanout(upstreams, { jsonrpc: '2.0', method: 'test', id: '1' }, 'first-success');
    ...
    it('aggregates responses with first-success strategy', async () => {
      const mockCaller = vi.fn(async (upstream) => ({
        upstream: upstream.name,
        response: upstream.name === 'primary'
  ...
  describe('executeFanoutFirstSuccess', () => {
    ...
    it('returns early when first upstream succeeds', async () => {
      ...
      setUpstreamCaller(mockCaller);
      const result = await executeFanoutFirstSuccess(upstreams, { jsonrpc: '2.0', method: 'test', id: '1' });
  ...
  describe('toJsonRpcRequest', () => {
    it('passes through valid JSON-RPC request', async () => {
      ...
      setUpstreamCaller(mockCaller);
      await executeFanout(upstreams, { jsonrpc: '2.0', method: 'tools/call', id: '1', params: {} }, 'first-success');
