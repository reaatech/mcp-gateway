...
import {
  ...
  withIdempotencyKey,
} from '../../src/mcp-client/retry-logic.js';
import type { RetryConfig, JsonRpcRequest, UpstreamResponse } from '../../src/mcp-client/types.js';

describe('calculateBackoff', () => {
  it('calculates exponential delay', () => {
    const config: RetryConfig = { maxRetries: 3, baseDelayMs: 100, maxDelayMs: 5000, jitter: false };
  ...
  it('caps delay at maxDelayMs', () => {
    const config: RetryConfig = { maxRetries: 10, baseDelayMs: 1000, maxDelayMs: 3000, jitter: false };
