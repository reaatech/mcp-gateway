  ...
  describe('loadGatewayConfig', () => {
    it('returns default config when no file exists', () => {
      setConfigLoaderDependencies({
        ...
        cwd: '/test/cwd',
        yaml: { load: () => { throw new Error('should not be called'); } },
      });
    ...
    it('parses valid yaml config', () => {
      const mockConfig = {
        ...
        rateLimits: { defaultRequestsPerMinute: 200, defaultRequestsPerDay: 20000, store: 'memory' as const },
        cache: { enabled: true, store: 'memory' as const, defaultTtlSeconds: 300 },
        audit: { enabled: false, storage: 'file' as const, filePath: '/tmp/audit', retentionDays: 30 },
    ...
    it('falls back to defaults on parse error', () => {
      const mockYaml = {
        load: () => { throw new Error('Parse error'); },
      };
