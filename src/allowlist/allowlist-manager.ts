...
import type { ToolAllowlist, AllowlistCheckResult } from './types.js';
  ...
  for (const pattern of tools) {
    if (matchesPattern(toolName, pattern)) {
      if (mode === 'allow') {
        ...
        return { allowed: false, reason: `Tool '${toolName}' is blocked by pattern '${pattern}'`, matchedPattern: pattern };
      }
    }
  }
