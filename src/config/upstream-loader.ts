...
function isLoopbackIP(ip: string, family: 'IPv4' | 'IPv6'): boolean {
  if (family === 'IPv4') {return ip.startsWith('127.') || ip === '0.0.0.0';}
  return ip === '::1' || ip === '::';
}
...
function isPrivateOrLocalhost(hostname: string): boolean {
  ...
  if (parsed) {
    if (isLoopbackIP(parsed.ip, parsed.family)) {return true;}
    if (parsed.family === 'IPv4' && isPrivateIPv4(parsed.ip)) {return true;}
    if (parsed.family === 'IPv6' && isPrivateIPv6(parsed.ip)) {return true;}
    return false;
  }
  ...
  return new Promise((resolve) => {
    ...
    dnsLookup(hostname, { family: 4 }, (err, address) => {
      ...
      if (err || !address) {
        ...
        dnsLookup(hostname, { family: 6 }, (err6, address6) => {
          ...
          if (isPrivateOrLocalhost(address6)) {
            resolve({ valid: false, reason: `SSRF protection: ${hostname} resolves to private IP ${address6}` });
            return;
          }
          resolve({ valid: true });
        });
      ...
      if (isPrivateOrLocalhost(address)) {
        resolve({ valid: false, reason: `SSRF protection: ${hostname} resolves to private IP ${address}` });
        return;
      }
      resolve({ valid: true });
    });
...
export async function validateUpstreamUrlAsync(urlString: string): Promise<UrlValidationResult> {
  try {
    ...
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
      return { valid: false, reason: `Invalid protocol: ${url.protocol}. Only http/https allowed.` };
    }
    ...
    if (isPrivateOrLocalhost(hostname)) {
      return { valid: false, reason: `SSRF protection: Upstream URL cannot point to localhost or private IP ranges (${hostname})` };
    }
