...
function getEvalFunction(client: unknown): RedisEvalClient | null {
  ...
  if (v4 && typeof v4 === 'object' && 'eval' in v4 && typeof (v4 as Record<string, unknown>)['eval'] === 'function') {
    const evalFn = v4['eval'] as (s: string, o?: { keys?: string[]; arguments?: string[] }) => Promise<unknown>;
    return {
      eval: (script: string, options?: { keys?: string[]; arguments?: string[] }) => evalFn(script, options),
    };
  }

  if ('eval' in client && typeof (client as Record<string, unknown>)['eval'] === 'function') {
    const evalFn = (client as Record<string, unknown>)['eval'] as (s: string, o?: { keys?: string[]; arguments?: string[] }) => Promise<unknown>;
    return {
      eval: (script: string, options?: { keys?: string[]; arguments?: string[] }) => evalFn(script, options),
    };
  }
...
export class RedisRateLimitStore implements RateLimitStore {
  ...
  async getRemaining(key: string, config: RateLimitConfig): Promise<number> {
    ...
    try {
      ...
      const refillRate = config.requestsPerMinute / 60000;
      const refilled = Math.min(tokens + elapsed * refillRate, config.burstSize ?? config.requestsPerMinute);

      // Also check daily quota if configured
      if (config.requestsPerDay > 0) {
        const quotaData = await this.client.hGetAll(quotaKey);
        if (quotaData && quotaData.count) {
          const dailyRemaining = Math.max(0, config.requestsPerDay - parseFloat(quotaData.count));
