 ...
 */
export function createAuthContext(options: Partial<AuthContext> & { tenantId: string }): AuthContext {
  ...
  if (options.keyName !== undefined) {ctx.keyName = options.keyName;}
  if (options.subject !== undefined) {ctx.subject = options.subject;}
  if (options.issuer !== undefined) {ctx.issuer = options.issuer;}
  if (options.expiresAt !== undefined) {ctx.expiresAt = options.expiresAt;}
  if (options.tokenFingerprint !== undefined) {ctx.tokenFingerprint = options.tokenFingerprint;}
