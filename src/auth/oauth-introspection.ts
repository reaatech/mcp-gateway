...
export async function introspectToken(
  ...
  try {
    ...
    if (!clientSecret) {
      return { valid: false, error: `OAuth client secret env variable '${clientSecretEnv}' is not set` };
    }
    ...
    if (!response.ok) {
      return { valid: false, error: `Introspection failed: ${response.status} ${response.statusText}` };
    }
