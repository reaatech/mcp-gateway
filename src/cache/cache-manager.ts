...
export class CacheManager {
  ...
  get(key: string): { hit: boolean; value?: unknown; ttlRemaining?: number } {
    ...
    if (result.value !== undefined) {
      ...
      const ret: { hit: boolean; value: unknown; ttlRemaining?: number } = { hit: true, value: result.value };
      if (ttl !== undefined) { ret.ttlRemaining = ttl; }
      return ret;
    }
    return { hit: false };
  }
   ...
   * Set in cache
   */
  set(key: string, value: unknown, ttlSeconds: number, metadata?: { tool?: string; tenantId?: string }): void {
  ...
  shouldBypass(headers?: Record<string, string>): boolean {
    ...
    if (headers?.['cache-control'] === 'no-cache') {return true;}
    return false;
  }
}
