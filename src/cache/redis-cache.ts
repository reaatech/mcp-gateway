...
export class RedisCache {
  ...
  constructor(client: RedisClient, prefix: string = 'mcp-gateway:cache:') {
    this.client = client;
    this.prefix = prefix;
  }
   ...
   * Set value in cache
   */
  async set(key: string, value: unknown, ttlSeconds: number, metadata?: Partial<CacheEntry>): Promise<void> {
    const fullKey = this.makeKey(key);
